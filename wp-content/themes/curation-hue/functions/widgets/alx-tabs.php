...
/* ------------------------------------ */
	private function _create_tabs($tabs,$count) {
		...
		$titles = array(
			'recent'	=> __('Recent Posts','curation-hue'),
			'popular'	=> __('Popular Posts','curation-hue'),
			'comments'	=> __('Recent Comments','curation-hue'),
			'tags'		=> __('Tags','curation-hue')
		);
		$icons = array(
		...
		<?php if($instance['recent_enable']) { // Recent posts enabled? ?>
			...
			<ul id="tab-recent" class="alx-tab group <?php if($instance['recent_thumbs']) { echo 'thumbs-enabled'; } ?>">
				...
				<?php if(curationhue_thumbnail(get_the_ID()) !=''): ?>
					<?php echo curationhue_thumbnail(get_the_ID(),'thumb-small'); ?>
							<?php else: ?>
			...
			<ul id="tab-comments" class="alx-tab group <?php if($instance['comments_avatars']) { echo 'avatars-enabled'; } ?>">
				...
				<li>
						...
						<div class="tab-item-inner group">
							...
							<div class="tab-item-name"><?php echo $comment->comment_author; ?> <?php _e('says:','curation-hue'); ?></div>
